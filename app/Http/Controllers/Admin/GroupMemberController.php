...
class GroupMemberController extends Controller
{
    ...
    public function addMember(Request $request, Group $group)
    {
        ...
        if ($group->users()->where('user_id', $user->id)->exists()) {
            return redirect()->route('admin.groups.show', $group)->with('error', $user->name . ' is already a member of this group.');
        }

        // Add user to the group
        $group->users()->attach($user);

        return redirect()->route('admin.groups.show', $group)->with('success', $user->name . ' added successfully!');
    }
    ...
    {
        return view('admin.groups.joingroup');
    }
    ...
    public function joinGroup(Request $request, Group $group)
    {
        ...
        return redirect()->route('admin.groups.show', $group)->with('success', 'You joined the group successfully!');
    }
    ...
    public function quitGroup(Request $request, Group $group)
    {
        if ($request->user()->groups()->where('groups.id', $group->id)->exists()) {
            ...
            return redirect(route('admin.groups.index'))->with('success', "You're no longer a member of $group->name Secret Santa.");
        }

        return redirect(route('admin.groups.index'))->with('error', 'You are not a member of this group.');
    }
    ...
    public function removeMember(Request $request, Group $group, User $user)
    {
        if ($user->groups()->where('groups.id', $group->id)->exists()) {
            ...
            return redirect(route('admin.groups.show', $group))->with('success', "$user->name was removed from $group->name Secret Santa.");
        }

        return redirect(route('admin.groups.show', $group))->with('error', 'You are not a member of this group.');
    }
